...
| nbUnreadsPerFeed hidden feed filter | **Strong** | Fixes a clear client/server mismatch -- the API returns feeds the sidebar doesn't render, causing an infinite notification loop. Small, safe change. [Issue filed upstream](https://github.com/FreshRSS/FreshRSS/issues/8694). |
